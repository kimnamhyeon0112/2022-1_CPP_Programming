...
protected:
	...
	char type;
public:
	Figure(POINT* _point, char _t) :point(_point), type(_t) { std::cout << "F" << std::endl; }
	virtual void Distance() {};
	virtual void Area() {};
	virtual double reArea() = 0;
	virtual double* reDistance() = 0;
	double operator+(Figure &t) {
		if (type == t.type) return this->area + t.area;
		else return -1;
	}
	double operator-(Figure &t) {
		if (type == t.type) return this->area + t.area;
...
public:
	Triangle(POINT a, POINT b, POINT c) :Figure(this->rePOINT(a, b, c), 't') { std::cout << "T" << std::endl; }
	void Distance() override { this->distance = this->reDistance(); }
	void Area() override { this->area = this->reArea(); };
	POINT* rePOINT(POINT a, POINT b, POINT c) { return new POINT[3]{ a, b, c }; }
	...
	double _distance(POINT a, POINT b) { return sqrt(pow(a.x - b.x, 2) + pow(a.y - b.y, 2)); }
	double reArea() override {
		...
		_s /= 2;
		double s = _s;
		for (double &d : dis) s *= (_s - d);
		s = sqrt(s);
		return s;
	}
	~Triangle() { std::cout << "~T" << std::endl; }
};
...
private:
	POINT b;
public:
	Circle(POINT a, POINT b) :Figure(this->rePOINT(a), 'c'), b(b) { std::cout << "C" << std::endl; }
	POINT* rePOINT(POINT a) { return new POINT[1]{ a }; }
